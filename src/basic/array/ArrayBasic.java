...
public class ArrayBasic {
  ...
  public static void main(String[] args) {
    ...
    // 생성할 때 배열의 크리를 지정해야 한다.
    arr = new int[5];

    // 3. 배열의 초기화 - 배열 내부에 실제 값들을 저장
    // 배열의 초기화는 인덱스를 이용합니다.
    // 인덱스: 배열 내부의 위치를 지정 할 수 있는 값.
    ...
    arr[2] = arr[0];
    arr[3] = (int) 3.14;
    // arr[4] = "들어갈까요?"; the array is int, so it won't accept a string.
    arr[4] = 100;
    ...
    // 7. 배열의 내부의 요소값을 문자열 형태로 한눈에 확인하기
    // 배열의 변수에는 배열의 주소가 들어있습니다.
    // System.out.println("arr = " + arr); (X) it will not show because the array is not a string
    ...
    // 9. 배열의 생성과 동시에 초기화도 하는 방벙
    char[] letters = new char[] {'A', 'B', 'C', 'D'};
    System.out.println("letters = " + Arrays.toString(letters));

    // 선언과 동시에 초기화를 할 때 닥 한번만 가능한 문법
    // 배열은 크기가 고정이기 때문에 늘어나거나 줄어들 수 없습니다.
    // 만약 배열의 크기를 놀리거나 줄이고 싶다면
    // 기존의 배열을 조작하는 것이 아니라 새롭게 생성해야 합니다.

    String[] names = {"홍일동", "김철수", "박영희"};
