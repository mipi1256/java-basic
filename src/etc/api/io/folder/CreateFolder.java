...
public class CreateFolder {
   ...
   public static void main(String[] args) {
      ...
      */
      ...
      if (!file.exists()) { // 해당 디렉토리 경로가 존재한다면 true, 존재하지 안는다면 false
         // file.mkdir(); -> 만들고자 하는 경로의 상위 디렉토리가 존재하지 않는다면 동작 안함
         file.mkdirs(); // 폴더 생성 -> 전부 다 만들어 줍니다. 여러 폴더 만들기!
         System.out.println("풀더 생성 완료!");
      } else {
      ...
      */
      ...
      /*
         When deleting folder is not recommended to write a code for it because
         it takes time and work. It's better to just go into the folder
         or the directory to delete the folder.
      */
      if (file.exists()) {
         file.delete();
         System.out.println("삭제 완료!");
      } else {
         System.out.println("삭제 실패 or 경로 존재하지 않음...");
      }
