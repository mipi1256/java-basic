...
public class HashMapExample {
   ...
   public static void main(String[] args) {
      ...
      */
      ...
      // There cannot be two elements with the same key and if there is one then the
      // value will get replace. However, if value is the same as other it will add it
      // Because the key is different.
      map.put("멍멍이", "김뽀삐"); // Key 중복
      map.put("어홍이", "홍길동"); // Value 중복
      System.out.println(map);

      // Map 내부에 key의 존재 유무를 확인하는 메서드: containsKey(key)
      // If it the input key has a value it will show true
      // if there is no value for the input key then it will show false.
      System.out.println(map.containsKey("야옹이"));
      System.out.println(map.containsKey("메뚜기"));

      // Map 내부의 값을 참조하는 법: get(key)
      // If the input key has a value it will show the value
      // If the input key doesn't have a value it will show null
      ...
      // If you want to clear the whole map
      map.clear();
      System.out.println(map);
