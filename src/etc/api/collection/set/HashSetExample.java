...
public class HashSetExample {
   ...
   public static void main(String[] args) {
      ...
      */
      ...
      // a better way to get all the elements inside a HashSet
      for (String s : set) {
         System.out.println(s);
      }

      // set에서 객체를 삭제: remove(객채) set doesn't have index so you have to remove them using the element name
      set.remove("JSP");
      System.out.println(set);

      // set을 list로 변환 -> 생성자의 매개값으로 set을 전달.
      // If you don't want to get multiple elements that are the same
      // Make a set first because it doesn't accept multiple of the same element
      // then you change the HashSet into an ArrayList
