...
public class FieldAndLocal {
   ...
   void printNumber(int c) { // 매개변수
      int b = 10; //지역 변수 -> 반드시 초기화를 수동으로 해야함. For local there is a need for a value in any variables
      System.out.println("필드 a = " + a);
      System.out.println("지연변수 b = " + b);
      System.out.println("매개변수 c = " + c);
   }

   void foo(){
      System.out.println("필드 a = " + a); // 필드는 어느 매서드에서든지 참조가 가능.
      // System.out.println("지연변수 b = " + b); (x) 지역변수. This variable only works in printNumber
   }
