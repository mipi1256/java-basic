...
public class OrderService implements AppService {
   ...
   private void rentalProcess(int movieNumber) {
      ...
      if (users.size() > 0) {
         ...
         if (userNums.contains(userNumber)) {
            // 대여 완료 처리
            User rentalUser = userRepository.findUserByNumber(userNumber); // 렌탈 유저 정보 확득.
            ...
            rentalUser.addOrder(newOrder); // 회원 대여 목럭에 주문을 추가.
   ...
   private void showRentalImpossibleList() {
      ...
      if (count > 0) {
         ...
         for (Movie movie : movieList) {
            ...
            System.out.printf("### 영화명: %s, 현재 대여자: %s(%s), 반남예정일: %s\n", movie.getMovieName(), rentalUser.getUserName(), lastPhoneNumber, rentalUser.getOrderList().get(movie.getSerialNumber()).getReturnDate());
   ...
   private void returnProcess(int userNumber) {
      ...
      // 영화 객체에서 화원정보를 삭제한다. -> rentalUser 필드 값을 null로 세팅
